...
test('lookup: deutscher Name, erste Marke, Inhalt; Quelle angegeben', async () => {
  const calls = [];
  const f = fakeFetch(
    { 'world.openfoodfacts.org': { '4316268476096': { product_name_de: ' Gouda  jung ', product_name: 'Young gouda', brands: 'Gutes Land, Netto', quantity: '400 g' } } },
    calls
  );
  assert.deepEqual(await Produktinfo.lookup('4316268476096', f), { name: 'Gouda jung', marke: 'Gutes Land', inhalt: '400 g', quelle: 'Open Food Facts' });
  assert.equal(calls.length, 1);
  assert.match(calls[0], /fields=product_name_de,product_name/);
});
...
test('lookup: ohne deutschen Namen den allgemeinen; sonst Open Beauty Facts; sonst null', async () => {
  ...
  assert.deepEqual(await Produktinfo.lookup('2', f), { name: 'Pflege Shampoo', marke: 'Hairwell', inhalt: '', quelle: 'Open Beauty Facts' });
...
test('describe: Bezeichnung eintragen, daraus automatisch die Warengruppe; CSV-Zeile mit Name', () => {
  ...
  const csv = Erfassung.toCSV(list.slice(0, 1)).split('\n')[1];
  assert.equal(csv, '4316268476096;Gouda jung;Gutes Land;400 g;;selbst gescannt;"Kamera-Scan; Open Food Facts";2026-09-25');
